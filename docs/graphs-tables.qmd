...
whale_meta <- read_csv("../data/smrt_hhmm_metadata.csv",
  ...
  mutate(tagon_loc = paste0(tagon_lat, ", ", tagon_lon),
...
whale_summary_table <- left_join(whale_summary_table0, 
                                 whale_meta,
                                by = "whale_ID") |>
  mutate(whale_num = as.numeric(factor(whale_ID))) |>
  # order cols
  select(whale_num, whale_ID, record_start_utc, tagon_loc,
         sensor_record_dur_hr, analyzed_duration,
         n_dc, n_typical, n_variant,
         n_mfas_dc, n_echo_dc, n_impulsive_dc, n_orca_dc, median_n_shal) |>
  rename(`Deployment Number` = whale_num,
         `Whale ID` = whale_ID,
...
sel_by_trans <- data |>
  filter(level == "1" & mfa_csel > 0) |>
  mutate(from_pretty = lag(coarse_state),
         cSEL = mfa_csel) |>
  rowwise() |>
  mutate(to_pretty = list(c("Typical", "Variant"))) |>
  ungroup() |>
  unnest(to_pretty) |>
  select(cSEL, coarse_state, from_pretty, to_pretty) |>
  drop_na() 
...
coarse_tpm_plot <- function(data, data0, title, ylab){
  ggplot(data, 
    ...
    geom_line() +
    geom_rug(aes(x = cSEL, y = NULL), data = sel_by_trans, sides = "b", alpha = 0.7) +
...
Rates of transition between states at the dive cycle scale. Rug plot on the x-axis indicates observed MFAS cSEL values, allocated to panels according to the most-likely decoded state sequence. 
:::
...
```{r}
sel_by_trans_fine <- data |>
  filter(level == "2" & fine_mfa_csel > 0) |>
  mutate(from_state = lag(state4),
         cSEL = fine_mfa_csel) |>
  rowwise() |>
  mutate(to_state = list(c(1,2,3,4))) |>
  ungroup() |>
  unnest(to_state) |>
  select(cSEL, coarse_state, from_state, to_state) |>
  drop_na() 
```
...
fine_tpm_plot <- function(data, data0, data_typical, title = ""){
  ggplot(data, aes(y = tp*100, x = SEL)) +
    geom_ribbon(aes(ymin = tp_lower*100, ymax = tp_upper*100), fill = "grey70") +
    geom_line() +
    geom_rug(aes(x = cSEL, y = NULL), 
             data = sel_by_trans_fine,
             sides = "b", 
             alpha = 0.2) +
...
gf_violin(value ~ type,
  ...
  gf_sina(valueNA ~ type,
          ...
          scale = "width",
          size = 0.5) |>
  gf_facet_wrap(~facet_labs, ncol = 2,
                scales = "free") |>
  gf_labs(x = "", y = "") |>
