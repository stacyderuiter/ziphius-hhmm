---
title: "Supplemental Materials For: Assessment of goose-beaked whale responses to mid-frequency active sonar using a hierarchical hidden Markov model"
author:
  - name: S.L. DeRuiter
  - name: E.A. Falcone
  - name: S.N. Coates
...
You can view this file online at: <https://stacyderuiter.github.io/ziphius-hhmm/supplemental-materials.html>.

The online html version includes interactive versions of many figures (allowing you to zoom), which the PDF does not include.
...
outw <- ifelse(knitr::is_html_output(),
               "100%",
               "85%")
...
knitr::opts_chunk$set(echo = FALSE, message = FALSE, warning = FALSE,
                      ...
                      fig.height = 2.5, # "65mm"
                      out.width = outw # trying to get caps on same pg in pdf 
                      )
...
if (!(knitr::is_html_output())) {
  ...
  tab_style(
    style = cell_text(size = pct(40)),
    locations = list(cells_body(), cells_column_labels()) 
  ) |>
  cols_width(
    `Whale ID` ~ pct(8),
    starts_with("Start") ~ pct(12),
    everything() ~ pct(5)
  )
}else{
...
variant_by_exposure_table <- L1_table |>
  mutate(`Previous State` = lag(State)) |>
  filter(`MFAS cSEL` != "0") |>
  select(`MFAS cSEL`,
         `MFAS RL`,
         `Sonar before EOE?`,
...
variant_by_exposure_table <- 
  ...
  mutate(
    `Variant State?` = ifelse(!is.na(variant_num), 'Yes', 'No')
    )
...
variant_by_exposure_table <- 
  ...
  mutate(
    `Variant State?` = ifelse(`Variant State?` == "Yes" &
                                     `Start Time (UTC)` != bout_start_time_UTC,
                                   "Started Earlier",
                                   `Variant State?`)
    ) |>
  select(-variant_num, -initial_csel, -bout_start_time_UTC) |>
  rename(
    `Switched to Variant State?` = `Variant State?`,
    `N Variant Dive Cycles` = n_dives,
...
Occurrence of Variant state for each dive cycle that contained detected MFAS sounds. The "State" column indicates the current dive-cycle-level state, while the "Switched to Variant State?" column indicates whether or not the whale switched from Typical to Variant state during the current dive cycle. In six cases, the whale was already *in* Variant state before the dive cycle with MFAS began, and then remained in Variant state during the exposed dive cycle; these rows have "Started Earlier" in the "Switched to Variant State?" column.
::::

Here, we see that whales transitioned to Variant state in `r sum(variant_by_exposure_table[,"Switched to Variant State?"] == "Yes")` out of `r nrow(variant_by_exposure_table)` MFAS-exposed dive cycles (`r round(100 * sum(variant_by_exposure_table[,"Switched to Variant State?"] == "Yes") / nrow(variant_by_exposure_table), digits = 1)` percent).

If we consider only dive cycles where the MFAS cSEL was above 110 dB re 1$\mu Pa^2*sec$, then that becomes `r sum(variant_by_exposure_table[,"Switched to Variant State?"] == "Yes" & variant_by_exposure_table[,"MFAS cSEL"] > 110)` out of `r nrow(variant_by_exposure_table[variant_by_exposure_table[,"MFAS cSEL"] > 110,])` MFAS-exposed dive cycles (`r round(100 * sum(variant_by_exposure_table[,"Switched to Variant State?"] == "Yes" & variant_by_exposure_table[,"MFAS cSEL"] > 110) / nrow(variant_by_exposure_table[variant_by_exposure_table[,"MFAS cSEL"] > 110,]), digits = 1)` percent).
...
data <- data |>
  ...
  mutate(dive_type = if_else(start_time_UTC <= (min(start_time_UTC) + lubridate::seconds(60*60*first(fd_dur_hours))),
                             "Foraging Dive",
                             "Non-Foraging Dive")) |> 
  ungroup()

fine_tally_by_divetype <- data |>
  group_by(coarse_state, state4, dive_type) |>
  summarise(`5-Minute Intervals` = n(),
            .groups = "drop") |>
  group_by(coarse_state, dive_type) |>
  mutate(Percent = round(100* `5-Minute Intervals` / sum(`5-Minute Intervals`), digits = 1),
         .groups = "drop") |> 
  rename(`Dive Cycle State` = coarse_state,
         `5-minute State` = state4,
         `Dive Type` = dive_type) |> 
  pivot_wider(id_cols = c("5-minute State", "Dive Type"),
              names_from = `Dive Cycle State`,
              names_sep = " ",
              values_from = c("5-Minute Intervals",
                              "Percent"),
              names_glue = "{`Dive Cycle State`} {.value}",)
# data |> 
#   group_by(dive_type, coarse_state) |> 
#   summarise(`Percent State 1` = round(100 * sum(state4 == 1)/n(), digits = 1), 
#             `Percent State 2` = round(100 * sum(state4 == 2)/n(), digits = 1),
#             `Percent State 3` = round(100 * sum(state4 == 3)/n(), digits = 1),
#             `Percent State 4` = round(100 * sum(state4 == 4)/n(), digits = 1),
#             .groups = 'drop') |>
#   rename(`Dive-Cycle State` = coarse_state,
#          `Dive Type` = dive_type) |>
#   gt()
...
:::: {#supptab-fine-tally-by-dive-type}

```{r}
if (knitr::is_html_output()){
  DT::datatable(fine_tally_by_divetype)
}else{
  gt::gt(fine_tally_by_divetype)
}
```

Occurrences of each of the four states at the five-minute timescale, by HHMM state and dive type (foraging or non-foraging).
::::
...
:::: {#suppfig-variant-scatter}


```{r}

gf_point(total_dur ~ n_dives, data = variant_summary) |>
  gf_labs(x = 'Number of Dive Cycles', y = 'Duration (h)')
...
gf_histogram(~response_dur_dives, data = sim_responses) |>
  ...
  gf_labs(x = 'Expected Response Duration (Dive Cycles)', y = 'N Simulated Whales')
...
`#set block(breakable: true)`{=typst}
...
plot_dive_profile <- function(whale, data, 
  ...
  this_whale_fine <- this_whale_fine |>
    mutate(state_starts = as.numeric(state4) - 
             dplyr::lag(as.numeric(state4), default = 0),
           last_state = lag(state4, default = NA)) 

  ngroups = sum(this_whale_fine$state_starts != 0)
  this_whale_fine$group_id = NA
  this_whale_fine$group_id[this_whale_fine$state_starts != 0] <- c(1:ngroups)
  
  xstarts <- this_whale_fine |> 
    filter(group_id != 0) |>
    mutate(state4 = last_state,
           start_time_UTC = start_time_UTC - lubridate::seconds(1),
           group_id = group_id - 1) |>
    drop_na(state4)
  
  this_whale_fine <- bind_rows(this_whale_fine,
                               xstarts) |>
    arrange(whale_ID, start_time_UTC)
    ...
    dive_trace <- 
      ggplot(this_whale_fine, mapping = aes(x = start_time_UTC,
                                            ...
                                            text = paste0("Time: ",
                                                          ...
                                                          "\nState: ",
                                                          state4))) +
      geom_path(show.legend = TRUE, linewidth = 0.5)  + 
  ...
  if (sum(!is.na(pull(this_whale_fine, mfas_rl))) == 0){
    ...
    rl_plot <- gf_point(mfas_rl ~ start_time_UTC, data = this_whale_fine,
                        size = 0.5, alpha = 0.5, color = 'grey40') |>
  ...
  if (show_explos){
      ...
      gf_point(50*fine_impulse_presence ~ start_time_UTC, 
               shape = 4, size = rug_size, alpha = 0.5, color = 'grey40')
  }
  
  if (show_echo){
    rl_plot <- rl_plot |>
      gf_point(50*fine_echo_presence ~ start_time_UTC, 
               shape = 1, size = rug_size, linewidth = 0.25, alpha = 0.5, color = 'grey40')
  }
  
  if (show_orca){
    rl_plot <- rl_plot |>
      gf_point(50*orca_presence ~ start_time_UTC, 
               shape = 2, size = rug_size, linewidth = 0.25, alpha = 0.5, color = 'grey40')
  }
  
  if (whale %in% c("Zica-20191012-144029","Zica-20191012-145101")){
    rl_plot <- rl_plot +
      geom_vline(xintercept = lubridate::ymd_hms("2019-10-13 05:58:00"),
                 linetype = "dashed",
                 color = "black") +
      geom_vline(xintercept = lubridate::ymd_hms("2019-10-14 13:14:00"),
                 linetype = "dashed",
                 color = "black")
  }
  ...
  if (!interactive){
    ...
    # interactive version 
    my_plot <- subplot(ggplotly(dive_trace), ggplotly(rl_plot),
            nrows = 2,
            heights = c(5/8,2/8),
            margin = 0.05,
            shareX = TRUE,
...
Dive depth profile for tag `r whales[w]`, with HHMM state and sound exposure information. Open circles indicate times echosounders were detected on the tag record. Vertical dotted lines in the lower panel indicate the time-period that the next plot zooms in on.
::::

#### Zoomed-in View for Zica-20191012-144029
...
Dive depth profile for tag `r whales[w]`, with HHMM state and sound exposure information. Symbols indicate times various sounds were detected on the tag record: open circles for echosounders and Xs for impulsive sounds. Vertical dotted lines in the lower panel indicate the time-period that the next plot zooms in on.
::::

#### Zoomed-in View for Zica-20191012-145101
...
Dive depth profile for tag `r whales[w]`, with HHMM state and sound exposure information. Symbols indicate times various sounds were detected on the tag record: open circles for echosounders and Xs for impulsive sounds.
::::
...
Dive depth profile for tag `r whales[w]`, with HHMM state and sound exposure information. Open circles indicate times echosounders were detected on the tag record.
::::
...
Dive depth profile for tag `r whales[w]`, with HHMM state and sound exposure information. 
::::
...
Dive depth profile for tag `r whales[w]`, with HHMM state and sound exposure information. Open triangles indicate times orca sounds were detected on the tag record.
::::
...
Dive depth profile for tag `r whales[w]`, with HHMM state and sound exposure information. Xs indicate times impulsive sounds were detected on the tag record.
::::
...
:::
<!-- the ::: should be end of landscape -->
...
data <- data |>
  mutate(wordy_state4 = paste0("State ", state4))

gf_sina(med_depth ~ coarse_state,
        colour = rgb(0.5,0.5,0.5, alpha = 0.2),
        # scale = "width",
        alpha = 0.05,
        data = data |> filter(level == "2" & fine_mfa_csel == 0)) |>
  gf_facet_wrap(~wordy_state4, ncol = 4) |>
  gf_theme(scale_color_viridis_c(bquote(Cumulative~MFAS~SEL~(dB~re~1*mu*Pa^2*sec)),
                        option = "magma", 
                        direction = -1)) |>
  gf_sina(med_depth ~ coarse_state,
        color = ~fine_mfa_csel,
  ...
  gf_labs(x = "Dive Cycle State",
          y = "Median Depth (m)") 
```

Depth distribution of 5-minute intervals, by dive-cycle and 5-minute state. Color indicates MFAS cSEL.
:::

\newpage

::: {#suppfig-fine-state-depth-csel-scatter}

```{r}
gf_point(med_depth ~ fine_mfa_csel,
         alpha = 0.1,
         data = data |>
          filter(level == "2")) |>
  gf_facet_grid(coarse_state ~ state4) |>
  gf_labs(y = "Median Depth (m)",
          x = "MFA cSEL")
```

Depth as a function of MFAS cSEL for 5-minute intervals, by dive-cycle and 5-minute state.
:::

## By Dive Type

::: {#suppfig-fine-dive-dist}
...
gf_sina(med_depth ~ coarse_state,
        ...
        alpha = 0.05,
        data = data |> filter(level == "2" & fine_mfa_csel == 0)) |>
  gf_facet_grid(dive_type ~ wordy_state4) |>
  gf_theme(scale_color_viridis_c(bquote(Cumulative~MFAS~SEL~(dB~re~1*mu*Pa^2*sec)),
                        option = "magma", 
                        direction = -1)) |>
  gf_sina(med_depth ~ coarse_state,
...
Depth distribution of 5-minute intervals, by dive-cycle and 5-minute state and dive type (foraging or non-foraging). Color indicates MFAS cSEL.
:::
